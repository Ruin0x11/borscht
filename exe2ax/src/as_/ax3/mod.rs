...
impl<'a> Ax3File<'a> {
    ...
    pub fn find_label(&self, label: &'a Ax3Label) -> ResolvedLabel {
...
fn get_jump_to_offset(primitive: &lexical::PrimitiveToken) -> u32 {
...
fn fix_if_else_stmts(nodes: &mut Vec<AstNode>) {
...
fn fix_else_stmt_labels<'a>(nodes: &mut Vec<AstNode>) {
    if nodes.len() < 3 {
        return;
    }
...
fn fix_dangling_else_stmts<'a>(nodes: &mut Vec<AstNode>) {
    if nodes.len() < 2 {
        return;
    }
...
fn resolve_scope(nodes: &mut Vec<AstNode>) {
...
/// program.
fn remove_trailing_stop(result: &mut Vec<AstNode>) {
...
impl<'a, R: Read + Seek> Parser<'a, R> {
    ...
    fn read_variable(&mut self) -> AstNode {
    ...
    fn read_block(&mut self, until: u32) -> AstNode {
        self.tab_count += 1;
    ...
    fn read_if_statement(&mut self) -> AstNode {
    ...
    pub fn read_on_event(&mut self) -> AstNode {
        let tok = self.tokens.next().unwrap();
        let token_offset = tok.token_offset;
    ...
    pub fn read_assignment(&mut self) -> AstNode {
    ...
    pub fn read_mcall(&mut self) -> AstNode {
        let primitive = self.tokens.next().unwrap();
        let token_offset = primitive.token_offset;
    ...
    fn add_early_decls(&mut self, result: &mut Vec<AstNode>) {
        for (i, dll) in self.file.dlls.iter().enumerate() {
            ...
            }).cloned().collect::<Vec<Ax3Function>>();
